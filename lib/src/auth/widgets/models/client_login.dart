...
class TeseLoginScreen extends StatefulWidget {
  ...
  const TeseLoginScreen({super.key, this.message});
...
class _TeseLoginScreenState extends StateMVC<TeseLoginScreen> {
  ...
  Future<void> _setInstalled() async {
    SharedPreferences pres = await SharedPreferences.getInstance();
    pres.setBool('installed', true);
  }
      ...
      children: [
        Image.asset(
          ...
          errorBuilder: (context, e, s) =>
              Icon(Icons.bolt, color: brandGreen, size: 60),
        ),
        const SizedBox(height: 15),
        const Text(
          "Welcome back",
  ...
  Widget _buildSubmitButton() {
    return CustomButtons.filledButton(
      text: 'Sign In',
      callback: () {
        if (_formKey.currentState!.validate()) {
          _formKey.currentState!.save();
          _con.loginUser({"email": email, "password": password});
        }
      },
    );
  }
  ...
  Widget _buildSignUpPrompt() {
    return Row(
      ...
      children: [
        ...
        TextButton(
          onPressed: () => Navigator.pushNamed(context, '/SelectCountry'),
          child: const Text(
            "Sign Up",
            style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),
          ),
