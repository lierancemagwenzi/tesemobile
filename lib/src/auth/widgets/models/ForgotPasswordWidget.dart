...
class ForgotPasswordWidget extends StatefulWidget {
  ...
  const ForgotPasswordWidget({super.key, this.message});
...
class _ForgotPasswordWidgetState extends StateMVC<ForgotPasswordWidget> {
  ...
  // Brand Colors consistent with your Registration flow
  final Color brandGreen = const Color(0xFF00D285);
  final Color primaryDark = const Color(0xFF1A0B2E);
  ...
  void initState() {
    ...
    if (widget.message != null) {
      Future.delayed(const Duration(seconds: 1), () {
        CustomMessageHandler().showSuccessSnakeBar(
          _con.scaffoldKey.currentContext ?? context,
          widget.message!,
        );
      });
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      resizeToAvoidBottomInset:
          true, // Set to true so keyboard doesn't hide the button
  ...
  Widget _buildHeader() {
    return Column(
      children: [
        // Using local Icon fallback in case logo-light isn't ready
        Image.asset(
          'assets/images/logo-light.png',
          height: 70,
          errorBuilder: (context, error, stackTrace) =>
              Icon(Icons.lock_reset_rounded, color: brandGreen, size: 60),
        ),
        const SizedBox(height: 20),
        const Text(
          "FORGOT PASSWORD",
  ...
  Widget _buildEmailField() {
    return TextFormField(
      onSaved: (value) => email = value!,
      validator: (value) =>
          Validator.validateEmail(value), // Use specific email validation
      style: const TextStyle(color: Colors.white),
      cursorColor: brandGreen,
      decoration: InputDecoration(
    ...
    return TextButton(
      ...
      child: RichText(
        text: TextSpan(
          ...
          children: [
            const TextSpan(text: "Remembered? "),
            TextSpan(
              text: "Login",
              style: TextStyle(color: brandGreen, fontWeight: FontWeight.bold),
            ),
