...
class _CreatePaymentLinkScreenState extends StateMVC<CreatePaymentLinkScreen> {
  ...
  // Controllers (Kept exactly as original)
  ...
  String? _selectedType = 'ONCE_OFF_FIXED';
  DateTime? _startDate = DateTime.now();
  DateTime? _endDate = DateTime.now().add(const Duration(days: 61));
  bool _allowOtherCurrencies = true;
  File? _imageFile;
  ...
  // --- Logic Methods (Restored from your original) ---
  ...
  void _submitForm() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final formData = {
        ...
        'paymentLinkType': _selectedType,
        'paymentLinkImageType': 'NEW',
        'paymentLinkStartDate': DateFormat('yyyy-MM-dd').format(_startDate!),
        'paymentLinkEndDate': DateFormat('yyyy-MM-dd').format(_endDate!),
        'paymentLinkCurrency': _selectedCurrency?.currencyCode,
        'paymentLinkCurrencyId': _selectedCurrency?.id,
        'paymentLinkOtherCurrencies': _allowOtherCurrencies.toString(),
      };
      ...
      if (result == true) {
        ...
        if (model != null) {
          ...
          CustomMessageHandler().showErrorSnakeBar(
            context,
            'Failed to create the payment link.',
          );
        }
      }
    }
  ...
  // --- UI Components with Dark/Light Mode Branding ---
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Form(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _buildSectionTitle("Basic Details", isDark),
                          _buildTextField(
                            controller: _nameController,
                            label: 'Payment Link Name',
                            icon: Icons.link,
                          ...
                          const SizedBox(height: 24),
                          _buildSectionTitle("Presentation", isDark),
                          _buildDescriptionField(isDark),
                          const SizedBox(height: 16),
                          _buildLogoUpload(isDark),

                          const SizedBox(height: 16),
                          _buildOtherCurrenciesSwitch(isDark),
  ...
  Widget _buildCurrencyDropdown(bool isDark) {
    return DropdownButtonFormField<CurrencyModel>(
      ...
      items: _con.currencies
          .map((c) => DropdownMenuItem(value: c, child: Text(c.currencyName)))
          .toList(),
  ...
  Widget _buildTypeDropdown(bool isDark) {
    return DropdownButtonFormField<String>(
      ...
      dropdownColor: isDark ? const Color(0xFF1A1A1A) : Colors.white,
      style: TextStyle(color: isDark ? Colors.white : Colors.black),
      decoration: _inputDecoration('Link Type', Icons.layers_outlined, isDark),
      items: _typeOptions
          .map(
            (t) =>
  ...
  Widget _buildLogoUpload(bool isDark) {
    return InkWell(
      onTap: _showImageSourceModal,
      child: DashedRectContainer(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Icon(Icons.cloud_upload_outlined, color: Colors.green.shade400),
              const SizedBox(height: 8),
              Text(
                _imageFile == null ? 'Tap to upload Logo' : 'Change Logo',
                style: TextStyle(
  ...
  Widget _buildOtherCurrenciesSwitch(bool isDark) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Switch(
            value: _allowOtherCurrencies,
            onChanged: (v) => setState(() => _allowOtherCurrencies = v),
            activeColor: Colors.green,
          ),
        ],
      ),
  ...
  // Restored Dialogs (Styled for Dark/Light)
  Future<bool?> showConfirmationDialog(BuildContext context) async {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to create this payment link?',
  ...
  void showSuccessDialog(BuildContext context, PaymentLinkModel model) {
    showDialog(
      context: context,
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Your payment link is now live and ready to use.'),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              Navigator.pushNamed(
                context,
                '/PaymentLinkDetails',
                arguments: {'link': model, 'isNew': true},
              );
            },
...
// Painter Classes (Kept exactly as original)
class DashedRectPainter extends CustomPainter {
  final Color color;
  final double radius;
  DashedRectPainter({required this.color, this.radius = 10.0});
  @override
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = color
